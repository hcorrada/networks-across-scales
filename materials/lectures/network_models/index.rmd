...
.column[
```{r adjplot4, echo=FALSE, fig.align="center", fig.width=5,eval=TRUE}
graph_1958 %>% 
  igraph::as_adj() %>%
  as.matrix() %>%
  as_data_frame() %>%
...
.column[
```{r centrality, echo=FALSE, fig.width=6,eval=TRUE}
...
.column[
```{r deg_centrality, fig.width=5, echo=FALSE,eval=TRUE}
undirected_graph_1958 %>%
  activate(nodes) %>% 
...
.column[
```{r eig_centrality, fig.width=5, echo=FALSE,eval=TRUE}
undirected_graph_1958 %>%
  activate(nodes) %>% 
