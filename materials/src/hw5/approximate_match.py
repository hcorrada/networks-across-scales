...
def partial_edit_distance(x, y, d):
    dist = 0
...
def find_approximate_matches(pattern, bwt, sa, d, text):
    ...
    candidates = zip(candidates, map(lambda x: partial_edit_distance(pattern, text[x:], d), candidates))
    candidates = filter(lambda x: x[1] <= d, candidates)
    return candidates
